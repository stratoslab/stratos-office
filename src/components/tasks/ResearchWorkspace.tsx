...
export default function ResearchWorkspace({ taskType }: ResearchWorkspaceProps) {
  ...
  if (offlineMode) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] p-4 md:p-6 gap-4">
        <div className="glass-panel rounded-2xl p-6 md:p-8 w-full max-w-md text-center" style={{ borderColor: 'rgba(245, 158, 11, 0.3)' }}>
          <div className="flex items-center justify-center gap-3 mb-4">
            <MaterialIcon name="lock" size={28} style={{ color: 'var(--warning)' }} />
            <h2 className="text-lg md:text-xl font-bold" style={{ color: 'var(--on-surface)' }}>Offline Mode Enabled</h2>
          </div>
          <p className="text-xs md:text-sm mb-6" style={{ color: 'var(--on-surface-variant)' }}>
          ...
          <button
            ...
            className="px-4 md:px-6 py-3 rounded-xl font-medium transition-colors min-h-[44px]"
  ...
  if (authError) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] p-4 md:p-6 gap-4">
        <div className="glass-panel rounded-2xl p-6 md:p-8 w-full max-w-md text-center" style={{ borderColor: 'rgba(239, 68, 68, 0.3)' }}>
          <div className="flex items-center justify-center gap-3 mb-4">
            <MaterialIcon name="error" size={28} style={{ color: 'var(--error)' }} />
            <h2 className="text-lg md:text-xl font-bold" style={{ color: 'var(--on-surface)' }}>API Key Invalid</h2>
          </div>
          <p className="text-xs md:text-sm mb-6" style={{ color: 'var(--on-surface-variant)' }}>
