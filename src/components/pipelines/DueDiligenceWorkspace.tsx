...
import { FinancialDataTable, RiskLevelBadge } from './PipelineOutputHelpers';
import MaterialIcon from '../ui/MaterialIcon';
import FileUploadZone from '../ui/FileUploadZone';

interface DueDiligenceWorkspaceProps {
  template: PipelineTemplate;
}

export default function DueDiligenceWorkspace({ template }: DueDiligenceWorkspaceProps) {
  ...
  const handleFinancialFile = useCallback((file: File, dataUrl: string) => {
    setFinancialFiles(prev => [...prev, file]);
      ...
      <div className="flex flex-col lg:flex-row gap-4 md:gap-6 h-full p-3 md:p-4 lg:p-0">
        <div className="lg:w-1/2 space-y-3 md:space-y-4">
          ...
          <div className="bg-[#0A2540]/50 rounded-xl p-4 md:p-6 space-y-4">
            ...
            <div>
              <label className="text-xs font-medium mb-1 block" style={{ color: 'var(--on-surface-variant)' }}>Financial Statements</label>
              <FileUploadZone taskType="financial_parser" onFile={handleFinancialFile} onError={() => {}} />
              {financialFiles.length > 0 && (
                <div className="mt-1 text-xs" style={{ color: 'var(--secondary)' }}>{financialFiles.length} file(s) uploaded</div>
              )}
            </div>
            ...
            <div>
              ...
              {contractFiles.length > 0 && (
                <div className="mt-1 text-xs" style={{ color: 'var(--secondary)' }}>{contractFiles.length} file(s) uploaded</div>
              )}
            </div>

            <button
              onClick={runPipeline}
              disabled={isRunning || (financialFiles.length === 0 && contractFiles.length === 0)}
            ...
            >
              {isRunning ? 'Running...' : 'Run Due Diligence'}
            </button>
          </div>
        </div>
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-4 md:gap-6 h-full p-3 md:p-4 lg:p-0">
      <div className="lg:w-1/2 space-y-3 md:space-y-4">
        ...
        <div className="bg-[#0A2540]/50 rounded-xl p-4 md:p-6">
          <h3 className="text-xs md:text-sm font-medium text-gray-400 mb-3">Step Outputs</h3>
          <div className="space-y-3">
            {run.steps.filter(s => s.status === 'complete').map((step) => (
              <div key={step.stepIndex} className="border border-white/10 rounded-lg p-3">
                ...
                {step.taskType === 'financial_parser' && step.parsedOutput && <FinancialDataTable data={step.parsedOutput} />}
                {step.taskType === 'contract_analyzer' && step.parsedOutput && (
                  <div>
                    {(step.parsedOutput as Record<string, unknown>)?.flagged_terms && (
                ...
                {step.taskType === 'legal_analyzer' && step.parsedOutput && (
                  <div className="space-y-1">
                    {((step.parsedOutput as Record<string, unknown>)?.risk_flags as Array<Record<string, unknown>>)?.map((f, i) => (
                      <div key={i} className="flex items-center gap-2 text-sm">
                ...
                {step.output && !step.parsedOutput && <p className="text-sm" style={{ color: 'var(--on-surface-variant)' }}>{step.output.slice(0, 200)}...</p>}
              </div>
            ))}
          </div>
        </div>
